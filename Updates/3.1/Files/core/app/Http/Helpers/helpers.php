...
function notify($user, $templateName, $shortCodes = null, $sendVia = null, $createLog = true)
{
    ...
    $notify->userColumn = $createLog ? $user->getForeignKey() : 'user_id';
    $notify->send();
}
...
function showRefList($refLists){
    // dd($refLists);
    $html = '';
    foreach ($refLists as $refUser) {
        $html .= "<li>$refUser->fullname ($refUser->username)";
        if($refUser->referrals->count()){
            $html .=  getRefHtml($refUser->referrals);
        }else{
            $html .= '<li>';
        }
    }
